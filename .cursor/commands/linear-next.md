---
description: Pick next unblocked actionable Linear issue, start it, and open a git worktree
---

# Linear next

Pick the **highest-priority**, **soonest-due** **unblocked** **actionable** issue in the
...
## 2) Actionable-only filter (exclude epics)

Before sorting, **remove** issues that are **not** suitable as a single
implementation branch (container / roadmap epics).

**Exclude** when the issue **title**, after trimming leading whitespace, matches
**`[Roadmap Epic]`** as a **case-insensitive** prefix (example: `[Roadmap Epic] Metadata …`).

If `list_issues` truncates titles, call **`get_issue`** for the candidate and
re-check the full title before treating it as actionable.

Do **not** infer epics from description text alone when the title does not match;
only skip when the **title** uses that epic prefix (or the team’s equivalent
convention documented in `docs/playbooks/linear-setup.md`).

## 3) Sort order (pick logic)
